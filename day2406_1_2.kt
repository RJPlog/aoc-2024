...
fun main() {
    ...
    /* 
            ...
            for (y in 0..9) {  // 102 -  -9
				for (x in 0..9) {  // 102 - 9    
					if (!(x == 4 && y == 6)) {   //  47 / 49       4 -6
						var steps = guard(2,x,y)
						if (steps == -1) {
                            result2 += 1
						    println("x: $x, y: $y, index: ${x+10*y} steps: $steps, result = $result2")
                        }
					}
				}
			}
	
            println("   ${result2} different positions could be choosen for an obstruction") //5822/5821/5304 to high 1288/1287 / 
